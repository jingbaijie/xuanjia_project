...
if(!ajaxConf){
	ajaxConf = {
	 //接口
	 webApiUrl: document.URL.match(/(https?)(:\/\/)[^\/]+/g)[0] +  "/webapi/",
	 //日志
	 logApiUrl: document.URL.match(/(https?)(:\/\/)[^\/]+/g)[0] + "/logapi/",
	 //图片
	 imgUrl: document.URL.match(/(https?)(:\/\/)[^\/]+/g)[0],
	}
}
...
var AjaxConfig = {
	//IP + 端口
	origin: document.URL.match(/(https?)(:\/\/)[^\/]+/g)[0],
	// 端口
	PORT: document.URL.match(/(:)(\d)+/g)[0].substr(1),
	// IP
	IP: document.URL.match(/(https?)(:\/\/)[^\:]+/g)[0],
	//图片地址
	imgUrl: ajaxConf.imgUrl || document.URL.match(/(https?)(:\/\/)[^\/]+/g)[0],
	//完整的项目地址 ip+端口+项目名
	projectUrl: document.URL.match(/(https?\:\/\/)([0-9a-zA-Z\_\.\:\-]*\/){2}/g)[0],
	//页面后台接口地址
	interfaceUrl: ajaxConf.webApiUrl + "web/",
	//活动接口
	actInterfaceUrl: ajaxConf.webApiUrl + "activity/",
	bobyfaceUrl: ajaxConf.webApiUrl + "baby/",
	//日志接口
	loggerUrl: ajaxConf.logApiUrl + "log/",
	//新增用户接口
	insertUserUrl: ajaxConf.webApiUrl + "web/insertUserInfo",
	//默认的首页地址，预备强制返回用
	defaultMainUrl: "/commPage?contentId=31&contentEName=newmain&contentCName=新版首页",
	...
	//全国日大数据志埋点接口
	saveCommonLog: ajaxConf.logApiUrl + "log/saveCommonLog",
	//全国日志大数据订购埋点接口
	saveCommonOrderLog: ajaxConf.logApiUrl + "log/saveCommonOrderLog",
	//大数据获取推荐片单接口 ， 根据用户： getRecommendMovieByUserId ； 根据片单： getRecommendMovieByMovieId 。
	bigDataRecCartoonUrl: ajaxConf.bigApiUrl + "movie/",
	//广东有线新增。
	//hostname 启动游戏主机名
	hostname: document.URL.match(/(https?)(:\/\/)[^\:]+/g)[0].split("://")[1],
	//启动游戏端口
	gamePort: 28080
