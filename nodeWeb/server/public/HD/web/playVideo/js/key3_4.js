...
function getKeyCodeName(keyCode) {
    for (var item in GXYX) {
        if (GXYX[item] == keyCode) {
            return item;
        }
    }
    for (var item in UT) {
...
var _keydown_flag_timer = undefined;
function keydownFlag_func() {
...
function keyDownEvent(evt) {
    ...
    // interface.loggerInfo('键值：' + keyCode + '；键名：' + keyName);

	if(keydownFlag_func()) {
        return;
    }
    clearTimeout(_keydown_flag_timer);
    _keydown_flag_timer =setTimeout(function(){
        keydown_flag = false;
    },2000);
    keydown_flag = true;
    ...
    switch (keyName) {
        ...
        case 0x1C:
            ...
            if (Page.playState == 'play' && CT.$("progressBar").style.visibility != "visible") {
                episodesFlag = setTimeout(function () {
                    ...
                    if (CT.$("recBackWrapper").style.visibility != 'visible') {
        ...
        case 0x1E:
            if (Page.playState == 'pause') {
                PAGE.focusHand(keyName);
            } else if (!isEpisodesHidden) {
                videoObj.leftOrRight(-1, 1);
            }
            break;
        case "RIGHT": // 向右拖动
        case 0x1F:
        ...
        case "VolumeQuiet": //mute静音
            ...
            break;
        case "VolumeUp": //音量++
            interface.loggerInfo('VolumeUp');
            videoObj.volUp(5);
            break;
        case "VolumeDown": //音量--
            interface.loggerInfo('VolumeDown');
            videoObj.volDown(5);
            break;
        case "FORWARD": //快进
            videoObj.forwardOrRewind(1);
