...
function allForStart(actionName){
    ...
    if(curPageNum){
        if(searchHavePrePage(pageMsgArr[curPageNum])){
            ...
            if(pageMsgArr[curPageNum] && searchHaveNextPage(pageMsgArr[curPageNum])){
                if(searchFromPageJsonByAction(pageMsgArr[curPageNum]["recommend_5"][0].commpageName)){
                    column.init();
                }else{
                    getNextPageMsgById(pageMsgArr[curPageNum]["recommend_5"][0].commpageId,function(){
...
function getPageMsgByAction(actionName,fn){
	if(actionName == basePageInfo.commonPageInfo.pageInfo.commPageEname){
        var data = basePageInfo.commonPageInfo;
...
function getPrePageMsgById(commPageId,fn){
    interface.findRecCommonPageInfo({contentId: commPageId},function(data){
		if(data){
			data = data.data;
                ...
                if(joinMsg[i].pageRecommendConfigTabVos[z].picLibraryTab && joinMsg[i].recommendDisplayType != 88){
                    switch (picType){
                        ...
                        case "0":
                            ...
                            picMsg.imgId = domNameArr[0] == "focus"?("generalFocus"+newRec.recommendId):(domNameArr[0]+ "Img" +newRec.recommendId);
                        ...
                        case "1":
                            ...
                            picMsg.imgId = domNameArr[1] == "focus"?("generalFocus"+newRec.recommendId):(domNameArr[1]+ "Img" +newRec.recommendId);
                        ...
                        case "2":
                            ...
                            picMsg.imgId = domNameArr[2] == "focus"?("generalFocus"+newRec.recommendId):(domNameArr[2]+ "Img" +newRec.recommendId);
                        ...
                        case "3":
                            ...
                            picMsg.imgId = domNameArr[3] == "focus"?("generalFocus"+newRec.recommendId):(domNameArr[3]+ "Img" +newRec.recommendId);
