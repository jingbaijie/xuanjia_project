...
/*var AjaxConfig = {
    //IP + 端口
    origin: document.URL.match(/(http|https)(:\/\/)[^\/]+/g)[0] || "http://10.69.52.56:28080/",
    // 端口
    PORT: document.URL.match(/(:)(\d)+/g)[0].substr(1),
    // IP
    IP: document.URL.match(/(http|https)(:\/\/)[^\:]+/g)[0],
    //图片地址
    imgUrl: document.URL.match(/(http|https)(:\/\/)[\w\.:]+/g)[0] || "http://180.96.20.178:28080/", //
    //完整的项目地址 ip+端口+项目名
    ...
    bobyfaceUrl: document.URL.match(/(http|https)(:\/\/)[^\/]+/g)[0] + "/iptv-web-api/baby/",
    //日志接口
    ...
    //默认的首页地址，预备强制返回用
    defaultMainUrl: "HD/web/mainPage/html/mainPage.html?contentId=31&contentEName=newmain&contentCName=新版首页",
    ...
    gamePort: 28080,
    //默认海报图片地址
    defaultCoverImg: document.URL.match(/(https?\:\/\/)([0-9a-zA-Z\_\.\:\-]*\/){2}/g)[0] + 'HD/images/defaultCover.png', //'http://10.69.52.57:28081',//

    tokenUrl: document.URL.match(/(http|https)(:\/\/)[^\/]+/g)[0] + "/order_gzgd/auth/",
};*/
...
function AjaxFun(option) {
    ...
    this.init = function(option) { //初始化
        ...
        this.Async = this.option.async == 'useFalse' ?  false : true;//true;//this.option.async == false ?  false : true ;//是否异步请求；true（异步）或 false（同步）
