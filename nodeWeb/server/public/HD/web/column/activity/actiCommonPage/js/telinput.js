 ...
 */
var actiContact = new actiContactModule();

function actiContactModule(actiContactName) {
	this.actiContactName = actiContactName || 'actiContact';
	if (!actiContactModule.actiContacts) actiContactModule.actiContacts = [];
	actiContactModule.actiContacts.push(this.actiContactName);
    //输入框焦点是否聚焦
     ...
     */
    this.init = function (main, option) {
        var _this = this;
        ...
        actiObj.getUserPhone(function (userInfoData) {
            if (userInfoData.successFlg == 1) {
                if (userInfoData.data.userPhone) {
                    _this.phoneNum = userInfoData.data.userPhone;
                } else {
                    _this.phoneNum = "";
                }
            }
    ...
    this.addButtons = function (contactFocusInfo, key) {
        var contactButtons = {
            ...
            otherFocusEvent: contactFocusInfo[key].otherFocusEvent ? "javascript:" +this.actiContactName+ "." + contactFocusInfo[key].otherFocusEvent + "()" : "",
            otherBlurEvent: contactFocusInfo[key].otherBlurEvent ? "javascript:" +this.actiContactName+ "." + contactFocusInfo[key].otherBlurEvent + "()" : "",
            clickHandler: contactFocusInfo[key].clickEvent ? "javascript:" +this.actiContactName+ "." + contactFocusInfo[key].clickEvent + "()" : "",
