function distanceCAL(_cool1,_cool2){
    var x1 = _cool1.x;
    var x2 = _cool1.y;
    var y1 = _cool2.x;
    var y2 = _cool2.y;
    var calX = x2 - x1;
    var calY = y2 - y1;
    return Math.random(Math.pow(calX*calX+calY*calY,0.5));
}
...
GeneralColumn.prototype = {
    ...
    * */
	focusMove: function(direction){
		...
		//PAGE.ProximityPrinciple(direction);
	...
	getNextFocusName: function(direction){
		...
		for (var key in PAGE.focusArr) {
			...
			// 忽略已经被禁用的焦点
			if (j.focusmodel.enFocus == true && j != curNode) {
				if(top < (y-h*2/3)){
					upNodeArr.push(j);
				}
				if(top > (y+h*2/3)){
    ...
    getUpFocusName: function(upNodeArr,curCoo){
        ...
        if (upNodeArr.length > 0) {
            for (var i = 0; i < upNodeArr.length; i++) {
				...
				if(nextCoo.x >= curCoo.x && nextCoo.x <= curCoo.x + curCoo.w){
					nextCoo.x += 10;
				}
				if(nextCoo.x <= curCoo.x && nextCoo.x + nextCoo.w >= curCoo.x){
					nextCoo.x -= 20;
				}
    ...
    getLeftFocusName: function(upNodeArr,curCoo){
		...
		var inCoo = false;
		if(nextCoo.y >= curCoo.y && nextCoo.y <= curCoo.y + curCoo.h){
			nextCoo.y += 20;
		}
		if(nextCoo.y <= curCoo.y && nextCoo.y + nextCoo.h >= curCoo.y){
			nextCoo.y -= 20;
		}
    ...
    getRightFocusName: function(upNodeArr,curCoo){
        ...
        if (upNodeArr.length > 0) {
            for (var i = 0; i < upNodeArr.length; i++) {
                // 生成坐标
                var nextCoo = new FunCoo(parseInt(upNodeArr[i].style.left), parseInt(upNodeArr[i].style.top)-20, parseInt(upNodeArr[i].style.width), parseInt(upNodeArr[i].style.height));
                // 计算距离
                //var jl = distanceCAL(curCoo, nextCoo);
                /*
				* 20180820
