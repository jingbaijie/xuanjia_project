 ...
 * name: districtObj
 * */
var baseUrl = basePageInfo.baseUrl;
var districtPageParams = {
    curPage: '',
    curFocusId: '',
    curClassifyId: 0
}
if (CT.getCookie('districtPageParams') != undefined && CT.getCookie('districtPageParams') != null && CT.getCookie('districtPageParams') != 'undefined' && CT.getCookie('districtPageParams') != 'null') {
    districtPageParams = CT.stringToJson(CT.getCookie('districtPageParams'))
    CT.delCookie('districtPageParams')
}
...
function District(obj) {
    ...
    /** ****************************/
    this.domTop = obj.domTop || this.boxTop || 120;                                       // 第一个元素的top
    this.domLeft = obj.domLeft || this.boxLeft || 508;                                    // 第一个元素的left
    ...
    this.arrowLeft = obj.arrowLeft || 621;                                               // 自定义箭头left
    ...
    this.curPage = districtPageParams.curPage || 0;                                       // 默认当前页数
    this.totalPage = 0;                                                                   // 总页数
    this.showTitle = obj.showTitle == false ? false : true;                             // 是否显示title
    /** *****************************/
    this.titleHeight = obj.titleHeight || 36;                                             // title的高
    this.imgUrl = obj.imgUrl ? AjaxConfig.imgUrl + obj.imgUrl : baseUrl + '/imgs/focus2.png';     // 焦点地址
    ...
    this.curClassifyId = districtPageParams.curClassifyId;                                // 默认分类                                                             // 当前分类
    /** *************路径记录****************/
    this.curFocusId = districtPageParams.curFocusId || 'hands_x0_y0_recommendFocus0_'     // 默认焦点
}
District.prototype = {
    ...
    getData: function (i) {
        var _this = this;
        _this.allData = districtPage.recommend_1;
        _this.len = _this.allData.length;
        _this.totalPage = Math.ceil(_this.len / (_this.step * _this.rowNum));
        // 渲染数据
        _this.showData();
    },
    generateContainer: function () {
        ...
        for (var i = 0; i < this.rowNum; i++) {    // 行
            for (var j = 0; j < this.step; j++) {  // 列
                if (this.showTitle) {
                    str = '<div id="recommendTitle' + (this.step * i + j) + '" style="width: ' + this.domWidth + 'px; height: ' + this.titleHeight + 'px; top: ' + (this.domHeight - this.titleHeight) + 'px; left: 0px;" class="recommendTitle"></div>'
                }
                var dom = '<div class="wrapper">' +
                    ...
                    this.domWidth + 'px;">' +
                    str +
                    '<div id="freeBadge' + (this.step * i + j) + '" style="top: 0;left: 0; width: 59px; height: 30px;background: url(' + baseUrl + '/imgs/freeBadge.png);visibility: hidden; border-radius:' + this.domBorderRadius + 'px 0 0 0;"></div>' +
                    '<div id="vipBadge' + (this.step * i + j) + '" style="position: absolute;top: 0;right: 0; width: 52px; height: 53px;background: url(' + baseUrl + '/imgs/vipBadge.png);visibility: hidden; border-radius:0 ' + this.domBorderRadius + 'px 0 0;"></div>' +
                    '</div>' +
                    ...
                    this.domHeight + 'px;width:' +
                    this.domWidth + 'px;">' +
                    '<img src="' + AjaxConfig.imgUrl + districtPage.recommend_1[i].recommendLabelpic.picPath + '" class="focusImage" id="recommendFocus' + (this.step * i + j) + '" alt="" style="visibility: hidden; width: ' + (this.domWidth) + 'px; height: ' + (this.domHeight) + 'px;">' +
                    '</div>' +
                    '</div>'
                doms += dom
                ...
                if (i == 0) {
                    btn.upEvent = 'javascript:districtObj.upPage(' + (this.step * i + j) + ')';
                }
                if (i == this.rowNum - 1) {
        ...
        if (this.showArrow) {
            ...
            var arrow = '<div id="downArrow" class="arrow" style="position: absolute;z-index: 2;visibility: hidden; top:' + this.arrowTop + 'px; left:' + this.arrowLeft + 'px;width: ' + this.arrowWidth + 'px; height: ' + this.arrowHeight + 'px;">' +
                '<img id="down" src="' + baseUrl + '/imgs/arrowDown0.png"/>' +
                '</div>';
            CT.$('districtContainer').innerHTML += arrow
            this.arrowChange()
        }
    ...
    arrowChange: function () {
        ...
        _this.timer = setTimeout(function () {
            CT.$('down').src = baseUrl + '/imgs/arrowDown' + _this.arrowCount + '.png';
            _this.arrowCount++;
    ...
    showData: function () {
        ...
        for (var i = 0; i < curData.length; i++) {
            ...
            try {
                recommendImg = AjaxConfig.imgUrl + curData[i].recommendPic.picPath;
    ...
    curData: function () {
        var _this = this;
        var curData = "";
        var startIndex = _this.step * _this.rowNum * _this.curPage;
    ...
    toContentDetailUrl: function (index) {
        ...
        districtPageParams = CT.jsonToString(districtPageParams)
        CT.setCookie('districtPageParams', districtPageParams)
    ...
    upPage: function (index) {
        ...
        this.curPage--;
        this.showData();
        var num = index + (this.rowNum - 1) * this.step;
        var start = PAGE.getFocusModel6("hands_x0_y0_recommendFocus" + num + "_");
        start.defaultFocus();
    },
    downPage: function (index) {
        if (this.curPage >= this.totalPage - 1) {
            ...
            this.curPage++;
            // 下一页焦点定位
            var curBtnNum = index % this.step;
            //还剩下多少未展示数据
            var haveNum = this.len - (this.rowNum * this.step * this.curPage);
            var maxBtnNum = haveNum >= this.step ? this.step - 1 : haveNum - 1;
            var num = curBtnNum <= maxBtnNum ? curBtnNum : maxBtnNum;
            var nextFocus = "hands_x0_y0_recommendFocus" + num + "_";
