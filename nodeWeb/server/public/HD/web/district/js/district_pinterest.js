...
var page = {
	...
	mainTop:reqQuery.mainTop || false,
    isOnPlay: true,
	/**
	 * 初始化页面，请求到页面需要的所有数据
	 */
    init : function () {
		...
		//瀑布流数据
		_this.pinterestData = commonPageInfo.recommend_1;
		//设置背景图、眉头
		_this.setBJAndNav(commonPageInfo);
		//创建推荐位的dom和光标
		_this.createImgDom();
		//各个推荐位的定位
		...
		//页面初始化焦点,实现来哪去哪先取眉头
		PAGE.focusInit();
		var defaultFocus = reqQuery.curFocusId || "hands_x0_y0_addContent0_";
     ...
     */
	 setBJAndNav:function(data){
		...
		//首页背景
		CT.$("BJ").src= AjaxConfig.imgUrl + data.pageInfo.pageTemplateBgpic.picPath;
	 },
    /**
     *   取more1数据,并返回一个对象
    ...
    isAllEquall : function (arr) {
     ...
     setImgDomPosition : function () {
        ...
        for(var i = 0,len = _this.pinterestData.length; i < len; i++){
			...
			CT.isBooleanUp(commonPageInfo.recommend_1[i].booleanUp,"hands_x0_y0_addContent"+i+"_",'contentBox'+i);
        }
    },
	/**
	 *  播放小视频
	...
	toJump:function(type,num){
        ...
        if(type.indexOf("bottom") > -1){  //瀑布流
		    CT.toAnterRecommendUrl(commonPageInfo,"recommend_1",num);
		}else if(type.indexOf("Mid")> -1){   //中间眉头
            //CT.writeInfo("中间眉头开始跳转>>>>>>>>>>>>");
		    CT.toAnterRecommendUrl(commonPageInfo.pageInfo.commonPageInfo,"recommend_2",num);
		}else if(type.indexOf("Right")> -1){   //右侧眉头
		    CT.toAnterRecommendUrl(commonPageInfo.pageInfo.commonPageInfo,"recommend_3",num);
		}
	}
};
page.init();
...
function backFunc(){
	if(navBarObj.showNavSelectedId == 0){
	    //推出拦截页
		if(commonPageInfo.recommend_2 && commonPageInfo.recommend_2[0]){
            CT.toAnterRecommendUrl(commonPageInfo,"recommend_2",0);
        }else{
            CT.commonJumpUrl(AjaxConfig.projectUrl + "/commPage?contentId=85");
        }
		//退出平台,添加退出日志
		//CT.exitPlatform();
	}else{
