(function () {
    ...
    videoObj.playByTime = function (time) {
        var _this = this;
        try {
            //机顶盒API
           videoPlayer.startSomePosPlay(time);
            videoPlayer.curStatus = "play";
        } catch (error) {
            videoPlayer.curStatus = "play";
            videoPlayer.curTime = time;
        }
    }
    ...
    videoObj.getMuteFlag = function () {
        ...
        try {
            //机顶盒api 获取当前静音值状态
            var muteFlag = videoPlayer.getVolume();
        } catch (error) {
        ...
        try {
            /*
                ...
                播放状态：
                    ...
                    返回 4：暂停播放；
            */
            var playMode = videoPlayer.getPlayerState();
            if (playMode == 4) {
                videoPlayer.curStatus = 'pause';
            }else if (playMode == 1) {
                videoPlayer.curStatus = 'play';
            }
            switch (videoPlayer.curStatus) {
                case "pause"://暂停
                playMode =  "pause";
                    break;
                case "play": //播放状态
