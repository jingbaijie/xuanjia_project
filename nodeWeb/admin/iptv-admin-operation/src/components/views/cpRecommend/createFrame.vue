...
import templateThemeInfo from "../cpRecommend/templateThemeInfo";
import recomConfigList from "../cpRecommend/recomConfigList";
import recomConfigDetail from "../cpRecommend/recomConfigDetail";
    ...
    checkTemplate() {
      this.$store
        .dispatch("axios_get_createSpecialUrl", {
          ...
          type: "10" // cpsp推荐的type9
        })
        .then(res => {
          if (res.data.errorCode == "1000") {
            eventBus.$emit("templateList3");
    ...
    publishTemplate() {
      if (this.actionType == "new") {
        ...
        axios
          ...
          // .post("sysadmin/content/special/add", d)
          .then(rs => {
  ...
  mounted() {
    ...
    eventBus.$on("setBgImg", form => {
      if (this.actionType == "new") {
        ...
        this.endSubmitData.templateInfo.id = form.commonPageId;
