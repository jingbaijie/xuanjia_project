  ...
  <div>
    ...
    >
      ...
      <el-table-column prop="status" label="状态">
        <template slot-scope="scope">{{dictionaryState[scope.row.status]}}</template>
      </el-table-column>
    ...
    <el-dialog v-dialogDrag title="字典数据" :visible.sync="dialogVisible" width="80%">
      <el-button
        v-if="isAuth('system:dict:insert')"
        :style="{'float':'right','margin':'0 70px 0 0'}"
        type="warning"
...
export default {
  ...
  destroyed() {
    eventBus.$off(["editGift", "addGift", "refreshDict"]);
  }
};
</script>
