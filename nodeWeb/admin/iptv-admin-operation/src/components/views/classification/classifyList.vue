<template>
  <div>
    <el-row>
      ...
      <el-form v-if="isAuth('system:type:list')" ref="form" class="form" :model="form">
        <el-form-item
          prop="parent"
          label="父级分类:"
          :label-width="formLabelWidth"
          v-if="form.parent"
        >{{form.parent.typeCname}}</el-form-item>
        <el-form-item
          prop="typeCname"
          label="分类名称:"
          :label-width="formLabelWidth"
        >{{form.typeCname}}</el-form-item>
        <el-form-item prop="labelPic" label="分类图片:" :label-width="formLabelWidth">
          <img v-if="labelPic" class="pic_icon" :src="imagesBaseUrl+labelPic" />
        </el-form-item>
        <el-form-item prop="detailPic" label="详情图:" :label-width="formLabelWidth">
          <img v-if="labelPic" class="pic_icon" :src="imagesBaseUrl+detailPic" />
        </el-form-item>
        <el-form-item prop="iconPic" label="缩略图:" :label-width="formLabelWidth">
          <img v-if="labelPic" class="pic_icon" :src="imagesBaseUrl+iconPic" />
        </el-form-item>
        <el-form-item prop="rankId" label="排行:" :label-width="formLabelWidth">{{form.rankId}}</el-form-item>
      </el-form>
  ...
  methods: {
    ...
    getTypeInfo(id) {
      this.$store
        ...
        .then(rs => {
          this.form = rs.data.data.info;
          this.form.detailPic == null
            ? (this.detailPic = {})
            : (this.detailPic = this.form.detailPic.picPath);
    ...
    handleEdit() {
      eventBus.$emit("editClassify", this.form);
    },
    handleDel() {
      console.log("del");
      this.$store
