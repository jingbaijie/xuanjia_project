  ...
  <transition name="fade-transform" mode="out-in">
    <el-tabs type="border-card" @tab-click="chageTab">
      <div :style="{'height':'50px','margin-left':'10px'}">
        <search id="header-search" class="right-menu-item" @searchKey="getSearch" />
        <div class="press_btn">
      ...
      <el-tab-pane :key="type.id" v-for="type in Imagestypes" :label="type.title">
        <ul class="selectImg">
          <li :key="img.id" v-for="img in imgData">
            <el-card :body-style="{ padding: '8px',  cursor:'pointer' }" shadow="hover">
              <div style="width: 100px;height: 20px;font-size: 12px">尺寸:{{img.picW}}×{{img.picH}}</div>
