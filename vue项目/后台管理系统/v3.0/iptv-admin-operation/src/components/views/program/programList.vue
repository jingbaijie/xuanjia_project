  ...
  <div>
      ...
      节目集列表 
        ...
        <div>
          ...
          >
            ...
            >
              <el-button
                v-if="isAuth('system:series:checkCopyrightPeriodMail')"
                :style="{ float: 'right', margin: '0 10px 0 0' }"
                size="mini"
                type="warning"
                plain
            ...
            <el-popover placement="top" trigger="click">
              <el-button
                v-if="isAuth('system:series:unifiedSearch')"
                size="mini"
                type="warning"
                plain
                @click="handleSearch('offline')"
                >下线</el-button
              >
              <el-button
                v-if="isAuth('system:series:unifiedSearch')"
            ...
            <el-popover placement="top" trigger="click">
              ...
              >
              <el-button
                v-if="isAuth('system:series:exportPlayUrl')"
                size="mini"
                type="warning"
                plain
            ...
            <el-popover
              v-if="isAuth('system:series:releaseAllTag')"
              placement="top"
              width="250"
              trigger="click"
            >
              <el-button
                ...
                type="warning"
                plain
                @click="handleReleaseTag(0)"
                >全部解除</el-button
              >
              <el-button
                v-if="isAuth('system:series:releaseTag')"
                size="mini"
                type="warning"
                plain
                @click="handleReleaseTag(1)"
                >单个解除</el-button
              >
              <el-button
                slot="reference"
            ...
            <self-button-search
              @getTypeInfo="config => getTypeInfoBySearch(curProductId, config)"
            />
            <keySearch
          ...
          >
              ...
              <template slot-scope="scope">
                ...
                >
                  ...
                  <el-button size="mini" slot="reference">{{
                    scope.row.tagsInfoList[0].typeCname
                  }}</el-button>
                </el-popover>
              </template>
            </el-table-column>
            ...
            <el-table-column label="CP信息">
              <template slot-scope="scope">
                <span v-if="scope.row.cspInfo">{{
                  scope.row.cspInfo.cpCname
                }}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="cartoonProprietorsStarttime"
              label="版权开始"
              width="100"
            ></el-table-column>
            <el-table-column
              prop="cartoonProprietorsEndtime"
              label="版权结束"
              width="100"
            >
              <template slot-scope="scope">
            ...
            <el-table-column
              prop="cspInfo.cpCname"
              label="版权方"
            ></el-table-column>
            <el-table-column
              prop="cartoonSumvideonum"
              label="集数"
            ></el-table-column>
            <el-table-column
            ...
            >
              <template slot-scope="scope">
                <el-input
                  ...
                  @keyup.enter.native="
                    submitRank(scope.row.rankId, scope.row.id)
                  "
                ></el-input>
              </template>
            </el-table-column>
            ...
            <el-table-column label="详情大图">
              <template slot-scope="scope">
                <img
                  :style="{ width: '50px' }"
                  v-if="scope.row.flagPic"
                  :src="imagesBaseUrl + scope.row.flagPic.picPath"
                />
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                ...
                >
                  <el-switch
                    ...
                    inactive-value="0"
                    active-color="#13ce66"
                    @change="setBatchSearch(scope.row,1)"
                  ></el-switch>
                </el-tooltip>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                ...
                >
                  ...
                  <el-dropdown-menu slot="dropdown">
                    ...
                    >
                    <el-dropdown-item
                      v-if="isAuth('system:series:update')"
                      command="edit"
    ...
    >
      ...
      >
        <el-breadcrumb-item
          :key="index"
          v-for="(item, index) in breadcrumb_child"
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="节目集信息" name="first">
          <div class="program_content">
            ...
            <div class="pro_line">
              ...
              <div class="pro_value">
                {{ "" + curProgram.cartoonIntroduction }}
              </div>
            </div>
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="节目信息" name="second">
          ...
          >
            ...
            <el-table-column align="center" label="中文名">
              <template slot-scope="scope">
                ...
                >
                  ...
                  <a :style="{ color: '#409eff' }" slot="reference">{{
                    scope.row.videoCname
                  }}</a>
                </el-popover>
              </template>
            </el-table-column>
            ...
            <el-table-column label="CP/SP信息" v-if="curCspInfo">{{
              curCspInfo.cpCname
            }}</el-table-column>
            <el-table-column prop="labelPic" label="标签图">
              <template slot-scope="scope">
                <img
                  :style="{ width: '50px' }"
                  v-if="scope.row.labelPic"
            ...
            <el-table-column prop="iconPic" label="详情图">
              <template slot-scope="scope">
                <img
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <el-button
                  ...
                  >编辑</el-button
                >
                <el-button @click="delListP(scope.row)" type="text" size="small"
                  >删除</el-button
                >
              </template>
            </el-table-column>
    ...
    >
      <el-form :inline="true" :model="tagForm" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button type="primary" @click="releaseTag">保存</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
import exportVideoInfo from "./exportVideoForm";
...
export default {
  ...
  data() {
    return {
      ...
      isRelated: false, //是关联还是解除
      relatedType: 0 //0:追加  1:覆盖
    };
  },
  watch: {
  ...
  methods: {
     ...
     *     1:单个
     */
    handleReleaseTag(type) {
      if (this.selectedIDs.length > 0) {
        if (type == 0) {
          this.$store
            .dispatch("axios_set_releaseAllTag", {
              cartoonIds: this.selectedIDs
            })
            .then(res => {
    ...
    getTag() {
      this.$store
        ...
        .then(rs => {
          this.tagList = this.getTreeData(rs.data.data.list);
        })
        .catch(er => {});
    },
    /**
    ...
    releaseTag() {
      if (this.isRelated) {
        ...
        this.$store
          .dispatch(api, {
            cartoonIds: this.selectedIDs,
            tagIds: this.tagForm.ids.join(",")
          })
          .then(response => {
            if (response.data.errorCode == "1000") {
              this.$message.success("关联成功");
              this.getTypeInfo();
              this.dialogReleaseTag = false;
            } else {
    ...
    handleSearch(action) {
      if (this.selectedIDs.length > 0) {
        this.$store
          .dispatch("axios_sub_unifiedSearch", {
            cartoonIds: this.selectedIDs,
            action: action
          })
          .then(res => {
    ...
    handleSelectionChange(val, type) {
      ...
      if (type) {
        val.forEach(element => {
          seriesList.push(element.cspInfo);
        });
      }
    ...
    handleBatchDel(ids, arr) {
      if (this.selectedIDs.length > 0) {
        ...
        if (newId == undefined || newId == "" || newId == []) {
          this.$message.error("当前勾选内容不可删！");
          return;
        }
        this.$confirm(
      ...
      if (this.selectedIDs.length > 0) {
        ...
        this.$store
          ...
          .then(response => {
            ...
            this.$refs.refPoppver.doClose();
            this.$refs.online.close();
            eventBus.$emit("refreshProgramList");
          })
          .catch(error => {});
      } else {
  ...
  beforeDestroy() {
    eventBus.$off(["getTypeInfo", "getProgramListBySeriesId"]);
  }
};
</script>
