  ...
  <div>
    ...
    >
      ...
      <el-table-column prop="levelId" label="归属分类">
        <template slot-scope="scope">{{
          templateClassify[scope.row.levelId]
        }}</template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      eventBus.$emit("addTemplate", this.activeData);
    },
    handleEdit(index, rows) {
      eventBus.$emit("editTemplate", rows[index], this.activeData);
    },
  ...
  destroyed() {
    eventBus.$off(["editTemplate", "addTemplate"]);
  }
};
</script>
