...
export default {
  ...
  methods: {
    setImg(item) {
      ...
      this.form.pageTemplatePicId =  item.id;
    },
    addTemplate() {
  ...
  mounted() {
    eventBus.$on("addTemplate", v => {
      this.activeData = v;
      this.addTemplate();
    });
    eventBus.$on("editTemplate", (v, a) => {
      this.activeData = a;
      this.editTemplate(v, a);
    });
  },
  components: { image_choice },
  beforeDestroy() {
    eventBus.$off("addTemplate");
    eventBus.$off("editTemplate");
  }
};
</script>
