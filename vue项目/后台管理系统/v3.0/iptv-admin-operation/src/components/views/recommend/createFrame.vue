  ...
  <div class="theme_activ">
    <div class="theme">
      ...
      <div style="height:100px;border-bottom: 2px dashed #eee;">
        <div class="dialog-footer">
          <el-button type="success" size="small" plain @click="goBack"
            >返 回</el-button
          >
        </div>
        <div class="dialog-footer">
          <el-button type="warning" size="small" plain @click="publishTemplate"
            >保 存</el-button
          >
        </div>
        <div class="dialog-footer">
          <el-button type="danger" size="small" plain @click="checkTemplate"
            >创建专项链接</el-button
          >
        </div>
      </div>
    ...
    <div class="theme_info">
      <el-form label-position="left">
        ...
        <el-form-item
          :label="activname"
          style="color:#409EFF;margin:20px 0 30px 20px;border-bottom: 2px dashed #eee; height:100px;text-align:center"
          :label-width="formLabelWidth"
        >
          <el-button
            v-if="activname"
        ...
        <el-form-item
          ...
          :label-width="formLabelWidth"
        >
          <el-button type="primary" @click="resetForm('recomConfigList')" plain
            >编 辑</el-button
          >
        </el-form-item>
        <el-form-item
          ...
          :label-width="formLabelWidth"
        >
          <el-input-number v-model="item.rankId" size="mini"></el-input-number>
        </el-form-item>
      </el-form>
      <!-- <div class="dialog-footer">
...
import templateThemeInfo from "../recommend/templateThemeInfo";
import recomConfigList from "../recommend/recomConfigList";
import recomConfigDetail from "../recommend/recomConfigDetail";
import bg from "@/assets/images/1038275mi7nbetip71bf81.jpg";
import axios from "@/common/axios";

export default {
  name: "createFrame",
  components: { templateThemeInfo, recomConfigList, recomConfigDetail },
  directives: {
    recomDrag: {
      bind(el, binding) {
  ...
  methods: {
    ...
    getTemplateById(id) {
      ...
      this.$store
        ...
        .then(req => {
          ...
          if (d.templateInfo.picLibraryTab) {
            this.imgurl =
    ...
    formateRecome(newList) {
      ...
      newList.forEach(item => {
        item.details.forEach(detail => {
          ...
          formateData.typeId.label = item.typeName;
          formateData.typeId["typeName"] = item.typeName;
          formateData.typeId.rankId = item.rankId;
          //初次获取归属类型
          let ty = this.$store.state.typeIds2;
          let re = false;
          ty.forEach(i => {
            if (i.value == item.typeId) {
              re = true;
            }
          });
          ...
          if (!re) {
            // 更新归属类型
            this.$store.commit("recomTypeList_setter", {
              value: item.typeId,
              label: item.typeName
            });
          }
          ...
          formateData.commonPageId = detail.commonPageId;
      ...
      this.$store
        ...
        .then(res => {
          if (res.data.errorCode == "1000") {
            eventBus.$emit("templateList2");
            this.$message.success("已创建专项链接！");
            eventBus.$emit("refreshTemplate");
    ...
    publishTemplate() {
      if (this.actionType == "new") {
        console.log(this.endSubmitData);
        this.endSubmitData.recommendInfo.sort(this.sortBy("typeId"));
        console.log(this.endSubmitData);
        axios
          .post("content/special/add", this.endSubmitData)
          .then(rs => {
        ...
        axios
          .post("content/special/update", this.editSubmitData)
          .then(rs => {
            if (rs.data.errorCode == "1000") {
              this.$message.success(
    ...
    concatDetails(attr) {
      ...
      var item = attr.pop();
      do {
        var key = item.id + item.name + item.age;
        if (obj[key]) {
          obj[key].details = obj[key].details.concat(item.details);
        } else {
          obj[key] = item;
        }
      } while ((item = attr.pop()));
  ...
  mounted() {
    ...
    eventBus.$on("setBgImg", form => {
      if (this.actionType == "new") {
        ...
        this.endSubmitData.pageInfo = form;
        this.endSubmitData.templateInfo.id = form.templateInfo.id;
    ...
    eventBus.$on("submitRecomeList", d => {
      let concatRecomList = this.concatDetails(d);
      this.editSubmitData.recommendInfo = concatRecomList;
      this.endSubmitData.recommendInfo = concatRecomList;
      this.tabView = "";
    });
