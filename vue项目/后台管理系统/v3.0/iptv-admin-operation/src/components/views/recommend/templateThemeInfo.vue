      ...
      >
        ...
        >
          <el-popover placement="right" width="535" trigger="hover">
            ...
            ></el-input>
            <img
              v-image="{ data: form.bgimg, picPath: curBgImg, fun: resetImg }"
              class="flagImage"
              slot="reference"
              :src="curBgImg"
            />
...
export default {
  ...
  methods: {
    ...
    onSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.form.commonPageId) {
            this.form.templateInfo.id = this.form.commonPageId;
          }
  ...
  created() {
    ...
    if (
      ...
      this.form.backgroundImage = (temp.bgimg && temp.bgimg.picPath) || "";

      if (this.form.backgroundImage == "") {
        this.curBgImg = defaultFocus;
      } else {
