 ...
 * @Description: 
 * @Author: jwzx
 * @Date: 2021-09-10 14:09:25
 * @LastEditTime: 2021-11-01 11:24:59
 * @LastEditors: jwzx
-->
<template>
  <div>
      ...
      <el-col :span="20">
        <div class>
          ...
          >
            ...
            />
            <self-button-search
              @getTypeInfo="keys => getTypeInfoBySearch(curProductId, keys)"
            />
              ...
              <template slot-scope="scope">
                ...
                >
                  ...
                  <el-button size="mini" slot="reference">{{
                    scope.row.typeInfoList[0].typeCname
                  }}</el-button>
                </el-popover>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDel(row) {
      let id;
      row ? (id = row.id) : (id = this.selectedIDs.join());
      this.$confirm("确定删除吗, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$store
            .dispatch("axios_del_game", { ids: id })
            .then(rs => {
