    ...
    <div class="theme_info">
      <el-form label-position="left">
        ...
        >
          <el-button
            type="warning"
            @click="resetForm('activeInfo', true)"
            circle
            size="mini"
            >编辑</el-button
          >
        ...
        >
          ...
          >
            ...
            <div :key="recomend.uid" v-for="recomend in focu.recomendPic">
              <el-checkbox v-model="recomend.checked">{{
                `未选中图：${recomend.picCname}`
              }}</el-checkbox>
              <div>
                X ：
                <el-input-number
            ...
            <div
              :key="recomendLabel.uid"
              v-for="recomendLabel in focu.recomendLabelPic"
            >
              <el-checkbox v-model="recomendLabel.checked">{{
                `选中图：${recomendLabel.picCname}`
              }}</el-checkbox>
              <div>
                X ：
                <el-input-number
            ...
            <div
              :key="recommendShow.uid"
              v-for="recommendShow in focu.recommendShowPic"
            >
              <el-checkbox v-model="recommendShow.checked">{{
                `选中展示图：${recommendShow.picCname}`
              }}</el-checkbox>
              <div>
                X ：
                <el-input-number
          ...
          <div v-if="item.fn.list" :style="{ 'margin-left': '-138px' }">
            ...
            <el-checkbox v-model="item.fn.list.checked">
              <span class="fn_name_title">{{
                `${listType[item.fn.list.type]}`
              }}</span>
            </el-checkbox>
            <div>
              X ：
          ...
          <div v-if="item.fn.inputPhone" :style="{ 'margin-left': '-138px' }">
            ...
            <el-checkbox v-model="item.fn.inputPhone.phone.checked">{{
              `输入框`
            }}</el-checkbox>
            <div>
              X ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.phone.xValue"
              ></el-input-number>
            </div>
            <div>
              Y ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.phone.yValue"
              ></el-input-number>
            </div>
            <div>
              宽 ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.phone.picW"
              ></el-input-number>
            </div>
            <div>
              高 ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.phone.picH"
              ></el-input-number>
            </div>

            <el-checkbox v-model="item.fn.inputPhone.tip.checked">{{
              `提示框`
            }}</el-checkbox>
            <div>
              X ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.tip.xValue"
              ></el-input-number>
            </div>
            <div>
              Y ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.tip.yValue"
              ></el-input-number>
            </div>
            <div>
              宽 ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.tip.picW"
              ></el-input-number>
            </div>
            <div>
              高 ：
              <el-input-number
                size="mini"
                v-model="item.fn.inputPhone.tip.picH"
              ></el-input-number>
            </div>
            <el-divider>
          ...
          >
            ...
            <span class="fn_name_title">{{
              `焦点ID:${ex.belongFocus} 限制数:${ex.limit}`
            }}</span>
            <el-divider>
  ...
  computed: {
    ...
    formatActiveData() {
      this.activeData.activityPages.forEach(page => {
        ...
        if (page.fn.exChange) {
          page.activityPageConfigs.push({
            attrJson: page.fn.exChange.join(),
            attrKey: "exChange",
            pageId: page.id
          });
        }
        ...
        if (page.fn.inputPhone) {
          page.activityPageConfigs.push({
            attrJson: page.fn.inputPhone.join(),
            attrKey: "inputPhone",
            pageId: page.id
          });
        }
