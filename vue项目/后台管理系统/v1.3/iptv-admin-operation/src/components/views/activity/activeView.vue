<template>
  <div class="theme_el">
    ...
    <div
      ...
      :id="item.uid"
      :style="{width:(item.picW/2)+'px', height:(item.picH/2)+'px',top:item.yValue/2+'px',left:item.xValue/2+'px'}"
    >
      <img
        v-if='item.checked'
        :src="item.url"
  ...
  methods: {
    ...
    renderViewImg() {
      ...
      this.pageInfo.fn.focus.forEach(f=>{
        f.recomendPic.length>0?f.recomendPic.forEach(p=>{this.imgArray.push(p)}):''
        f.recomendLabelPic.length>0?f.recomendLabelPic.forEach(p=>{this.imgArray.push(p)}):''
        f.recommendShowPic.length>0?f.recommendShowPic.forEach(p=>{this.imgArray.push(p)}):''
