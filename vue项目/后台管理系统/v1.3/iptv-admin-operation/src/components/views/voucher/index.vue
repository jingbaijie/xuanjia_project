<template>
  <div>
    <div style="btn_tab">
      ...
      <el-input class="searchInput" style placeholder="根据用户名或卡号查询" v-model="searchValue"></el-input>
          <el-button
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column fixed prop="totalDay" label="有效期（天）"></el-table-column>
      ...
      <el-table-column fixed prop="activeDate" label="激活时间">
         <template slot-scope="scope">{{spileDate(scope.row.activeDate)}}</template>
      </el-table-column>
      <el-table-column prop="createTime" label="激活开始时间">
         <template slot-scope="scope">{{spileDate(scope.row.createTime)}}</template>
      </el-table-column>
      ...
      <el-table-column prop="effectiveStartTime" label="有效期开始时间">
         <template slot-scope="scope">{{spileDate(scope.row.effectiveStartTime)}}</template>
      </el-table-column>
      <el-table-column prop="effectiveEndTime" label="有效期结束时间">
         <template slot-scope="scope">{{spileDate(scope.row.effectiveEndTime)}}</template>
      </el-table-column>
