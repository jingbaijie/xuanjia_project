function GeneralColumn() {
    ...
    //第一个推荐信息位置
    this.firstRecommendPicLeft = parseInt(CT.getCookie("firstRecommendPicLeft")) || 26;
    /**
         * 页面初始化
         */
    this.init = function () {
        ...
        if (typeof (focusMsg) == "object" && focusMsg.length > 0) {
            ...
            for (var i = 0; i < focusMsg.length; i++) {
                ...
                if (focusMsg[i].recommendLabelpic.length > 0) {
                    ...
                    focusId = "hands_x0_y0_" + focusName + focusMsg[i].recommendId + "_";
                    imgId = focusName + focusMsg[i].recommendId;
                    imgSrc = "HD/web/column/columnLargeScreenSwitch/img/empty.png";
                    focusTop = "520px";
                    focusLeft = this.firstRecommendPicLeft + (parseInt(focusMsg[i].recommendPic[0].width) + 24) * i + "px";
                    focusHtml += "<div id = '" + focusId + "' style = 'position:absolute;left:" + focusLeft + ";top:" + focusTop + ";'>" +
                        "<img id = '" + imgId + "' src = '" + imgSrc + "' style = 'visibility: hidden;'></div>";
                    this.addRecButtons(i, focusMsg[i], focusName, clickFnName, otherFocusFn, otherBlurFn);
                }
            }
            return focusHtml;
	...
	* */
        ...
        if (typeof (detailMsg) == "object" && detailMsg.length > 0) {
            ...
            for (var i = 0; i < detailMsg.length; i++) {
                if (detailMsg[i][recommendType].length > 0) {
                    ...
                    if (recommendType == "recommendPic") {
                        detailTop = "520px";
                        detailLeft = this.firstRecommendPicLeft + (parseInt(detailMsg[i][recommendType][0].width) + 24) * i + "px";
                    }else if(recommendType == "recommendShowPic"){
                        detailTop = "0px";
                        detailLeft = "0px";
                    }
    ...
    this.moveEvent = function (direction, ii) {
        ...
        if (direction == "left") {
            //焦点在页面最左侧
            if (parseInt(CT.$(curFocus.FocusID).style.left) <= 26) {
                //焦点不是第一个
                if (ii > 0) {
                    for (i = 0; i < recommendMsg.length; i++) {
                        ...
                        CT.$("hands_x0_y0_generalFocus" + recommendMsg[i].recommendId + "_").style.left = focusDOMleft + parseInt(recommendMsg[i].recommendPic[0].width) + 24 + "px";
                        CT.$("focusPic" + recommendMsg[i].recommendId).style.left = recommendPicLeft + parseInt(recommendMsg[i].recommendPic[0].width) + 24 + "px";
                    }
                }
            }
            if (ii > 0) {
            ...
            //焦点在页面最右侧
            if (parseInt(CT.$(curFocus.FocusID).style.left) >= 26 + (parseInt(recommendMsg[0].recommendPic[0].width) + 24) * 3) {
                //焦点不是最后一个
