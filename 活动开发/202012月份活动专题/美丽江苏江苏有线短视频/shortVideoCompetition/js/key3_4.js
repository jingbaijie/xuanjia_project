...
function getKeyCodeName(keyCode) {
    ...
    for (var item in BJGH) {
        if (BJGH[item] == keyCode) {
            return item;
        }
    }
    for (var item in HH) {
    ...
    for (var item in GDBC) {
        if (GDBC[item] == keyCode) {
            return item;
        }
    }
    for (var item in GDBC_BEIFEN) {
        if (GDBC_BEIFEN[item] == keyCode) {
            return item;
        }
    }
	for (var item in GDBC_VOLUMN) {
        if (GDBC_VOLUMN[item] == keyCode) {
            return item;
        }
    }
	for (var item in GZBC) {
        if (GZBC[item] == keyCode) {
            return item;
        }
    }
}
...
evt = evt != null && evt != undefined ? evt : window.event;
            evt.preventDefault();
                if(document.URL.indexOf("columnGeneralPortal.html") > -1 || document.URL.indexOf("interceptePage.html") > -1||document.URL.indexOf("columnGeneral_out") > -1||document.URL.indexOf("activity_saleStudyBox") > -1){
...
//88.6 JS消息注册
function MediaPlayerEvent(event){
    var keycode = event.keyCode | event.which
    var messageId = event.userInt;
    switch(keycode){
        case 768:
            var MediaEvent=Utility.getEvent(768,messageId);
            break;
        case 40408:
            var MediaEvent2=Utility.getEvent(40408,messageId);
            break;
    }
}
// document.onkeypress=MediaPlayerEvent;


var navPage = {
    midToNextPage: function(){

    },
    midToPrePage: function () {
