...
function signLottery() {
	...
	this.exchangeStandard = 110; //兑换标准
...
signLottery.prototype = {
	...
	exchangeFunc: function() {
		...
		if(_this.isExchange) {
			...
			//当前用户获奖信息
			actiObj.getUserPrizeInfo(13, function(res) {
				if(res.successFlg == 1) {
					_this.isFusai = true;
				}
			});
			//当前奖品信息			
			actiObj.getActivityPrize(13, function(res) {
				if(res.errorCode == "1000") {
		...
		if(this.isExchange) {
			...
			if(!_this.isFusai) { //没有兑换过
				if(_this.fusaiRemainNum > 0) { //复赛剩余名额
					actiObj.setPrize(13, _this.fusaiId, function(res) {
						//兑换成功
