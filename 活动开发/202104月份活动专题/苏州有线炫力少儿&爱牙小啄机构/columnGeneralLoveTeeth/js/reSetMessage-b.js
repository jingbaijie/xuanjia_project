...
var updateMessage = {
	...
	getNumsRecMsg: function(joinMsg){
		...
		for(var i = 0; i < joinMsg.length; i++){
            ...
            var tabVos = joinMsg[i].pageRecommendConfigTabVos;
            for(var z = 0; z < tabVos.length;z++){
                if(tabVos[z].picLibraryTab){
					
                    var picType = tabVos[z].picType;
                    var picMsg = {
                        divId: "",
                        imgId: "",
                        url: tabVos[z].picLibraryTab.picPath,
                        height: tabVos[z].picLibraryTab.picH,
                        width: tabVos[z].picLibraryTab.picW,
                        xValue: tabVos[z].xValue || tabVos[z].xvalue,
                        yValue: tabVos[z].yValue || tabVos[z].yvalue
                    };
                    switch (picType){
                        case 0:
                        case "0":
