...
var ipArr = [6438,0,3440,0,5992,0,5898,6358];
var stop = [0,2,4,6,7];
...
orderJs.columnGetAuth(function(data){
    actiObj.getChance(function (res) {//进入首页时获得机会
        ...
        if(data == "0"){
            isOrder = 0;
            CT.$("chanceNum").innerHTML = 1000-chance;
        }else{
            isOrder = 1;
...
function init() {
    PAGE.focusInit();
    curFocus = PAGE.getFocusModel6("hands_x0_y0_startFocus_");
...
function directChange() {//选中框位置改变
    ...
    if(probability < 999 && prizeNum > 0 && prizedegree > 0 && !userHavePrize){
		userHavePrize = true;
