...
function startLottery() {
	if(creditNum <= 0) {
		CT.$("lotteryTan").src = "img/tanChuang/noChou.png";
		...
		//每个奖品对应的位置	
		var moveLottery = document.getElementsByClassName('moveLottery');
		var prizeDeg = [];
		...
		ringInterval = setInterval(function() {
			if(ringNum <= 1) {
				...
				if(circleImg > 7) {
					ringNum--;
					moveLottery[circleImg - 1].style.opacity = 0;
					circleImg = 0;
					if(ringNum != 1) {
						moveLottery[circleImg].style.opacity = 1;
					}
				} else {
					moveLottery[circleImg].style.opacity = 1;
					if(circleImg >= 1) {
...
function showConfirm() {
	actiObj.changeFocus("hands_x0_y0_lotteryTanFocus_");
	CT.$("lotteryTan").src = "img/tanChuang/nochouzhong.png";
}
