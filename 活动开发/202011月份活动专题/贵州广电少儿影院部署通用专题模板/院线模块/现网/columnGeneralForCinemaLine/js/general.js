            ...
            for(var i = 0; i < detailMsg.length; i++){
            	if(detailMsg[i][recommendType].length > 0){
                    ...
                    detailHtml += "<div id = '" + detailId +"' style = 'width: "+ detailWith +";height: "+ detailHeight
                        +";position: absolute;top: "+ detailTop +";left: "+ detailLeft +";z-index: "+ detailZIndex +";' attr-top = '"+ detailTop +"' attr-left = '"+ detailLeft +"' >" +
                        "<img id = '"+ imgId +"' src = '"+ imgSrc +"' style = 'width: "+ detailWith +";height: "+ detailHeight +";visibility: "+ visi +";z-index: "+ detailZIndex +";'>"
                        + "</div>";
				}
            }
            return detailHtml;
    ...
    * */
	focusMove: function(direction){
		if(curFocus.TempData){
            ...
            switch (direction){
                case "up":
					if(curFocus.TempData.focusMoveEvent.length > 0 && curFocus.TempData.focusMoveEvent[0] == -1) {
                        return;
                    }
                    ...
                    break;
				case "down":
                    if(curFocus.TempData.focusMoveEvent.length > 0 && curFocus.TempData.focusMoveEvent[1] == -1){
                        return;
                    }
                    ...
                    break;
				case "left":
					if(curFocus.TempData.focusMoveEvent.length > 0 && curFocus.TempData.focusMoveEvent[2] == -1){
                        return;
                    }
                    ...
                    break;
				case "right":
					if(curFocus.TempData.focusMoveEvent.length > 0 && curFocus.TempData.focusMoveEvent[3] == -1){
                        return;
                    }
