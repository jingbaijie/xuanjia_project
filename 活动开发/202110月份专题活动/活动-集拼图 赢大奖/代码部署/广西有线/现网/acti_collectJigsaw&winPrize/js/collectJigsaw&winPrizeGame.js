...
function collectJigSawGame() {
    ...
    this.isOrder = CT.requestValue("isOrder") || 0;//是否订购活动
...
collectJigSawGame.prototype = {
    ...
    init: function () {
        ...
        actiObj.getUserDataList(function (userData) {
            if (userData.errorCode == 1000 && _this.restart != 1) {
                var userDataArr = JSON.parse(userData.data.userActiData);
                        ...
                        actiContact.init("prizeTip", option, function () {
                            ...
                            if (_this.isOrder == 0) {//未订购活动
                                if (_this.currentPiecesNum == 0) {//当前用户是0个碎片，弹出锦鲤卡，获得9个碎片
                                    ...
                                    actiObj.setUserDataList(JSON.stringify({ "seenSheet": _this.seenSheetArr, "date": _this.lastDate, "totalPieces": _this.currentPiecesNum, "currentClue": _this.currentClueNum, "prizeIndex": _this.prizeIndex }));
                                    //拼图进度展示
    ...
    backFromSheetJudge: function () {
        ...
        interface.getSystemConfigByKey(function (configRes) {
            if (configRes.errorCode == 1000) {
                ...
                _this.getPiecesClueOdds = JSON.parse(configRes.data.configValue)[index].get;
                _this.stolenClueOdds = JSON.parse(configRes.data.configValue)[index].stolen;
    ...
    jumpToSheet: function (index) {
        ...
        actiObj.setUserDataList(JSON.stringify({ "seenSheet": this.seenSheetArr, "date": nowDate, "totalPieces": this.currentPiecesNum, "currentClue": this.currentClueNum, "prizeIndex": this.prizeIndex }), function (res) {
            if (res.successFlg == 1) {
                // 上传本次跳转片单状态
                actiObj.setUserCredit(_this.lastSheet, function (creditData) {
                    if (creditData.successFlg == 1) {
                        PAGE.otherPageParam = "&isOrder=" + _this.isOrder + "&prizeIndex=" + _this.prizeIndex + "&activityId=" + CT.requestValue("activityId") + "&contentCName=" + CT.requestValue("contentCName") + "&contentId=" + CT.requestValue("contentId") + "&contentEName=" + CT.requestValue("contentEName") + "";
                        CT.goPage();
                        CT.toAnterRecommendUrlTrue(_this.bottomRecList[_this.sheetShowArr[index]], 1);
                    }
                });
            }
        });
    ...
    goodLuckCardShow: function () {
        ...
        actiObj.setUserDataList(JSON.stringify({ "seenSheet": this.seenSheetArr, "date": this.lastDate, "totalPieces": this.currentPiecesNum, "currentClue": this.currentClueNum, "prizeIndex": this.prizeIndex }));
