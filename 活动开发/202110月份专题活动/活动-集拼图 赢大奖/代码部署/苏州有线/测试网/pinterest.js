...
var page = {
    ...
    comPareArrayIndex: 0,
    //默认初始Left值
    defaultFocus: "hands_x0_y0_DMSYvideo_",// "hands_x0_y0_DMSYKW0_",
    //默认初始Left值
    defaultLeft: 40,
    //默认初始元素上下间隔值
    defaultSpaceCol: 20,
    ...
    init: function () {
        ...
        //             cache_strategy: 2
        //         };
        //     xjDataLog.getUserId = function(){return "001"};
        //     }
        // }catch(e){}
        _this.initPageDom();
    },
    getChanceFunc: function () {
        ...
        chance.getChance(parseInt(xjDataLog.getUserId()), 206, function (getActi1Res) {
            if (getActi1Res.data.activityChance >= 1) {
                chance.getChance(parseInt(xjDataLog.getUserId()), 205, function (getActi2Res) {
                    if (getActi2Res.data.activityChance >= 1) {
                        return;
                    } else {
                        chance.setChance(parseInt(xjDataLog.getUserId()), 205, function (setActi2Res) {
                ...
                chance.setChance(parseInt(xjDataLog.getUserId()), 206, function (setActi1Res) {
                    CT.getActivityUrl(206);
                });
            }
        });
    },
    ...
    createTheme: function (callBack) {
        ...
        if (!CT.$('themeListAlert')) {
            ...
            for (var index = 0; index < _this.themeListArr.length; index++) {
                listBtnStartTop = 270 + index * parseInt(268 / _this.themeListArr.length);
        ...
        if (_this['smallVideoListInfo' + _this.CurCartoonId]) {
            ...
            try {
                var playParams = {
                    ...
                    playUrl: _this['smallVideoListInfo' + _this.CurCartoonId].data[0].movieDetails[0].playUrl, //合集媒资
                    videoIndex: 0,//当前集数下标
                    //上传小视频播放日志所用信息
            ...
            interface.findVideoListByCartoonId({
                ...
                try {
                    var playParams = {
                        ...
                        height: _this.playObj.h,
                        width: _this.playObj.w,
                        playUrl: res.data[0].movieDetails[0].playUrl, //合集媒资
                        videoIndex: 0,//当前集数下标
                        //上传小视频播放日志所用信息
    ...
    toJump: function (type, num) {
        ...
        if (CT.dataType(num) != 'object' && _this.moer1DataJson[num] && _this.moer1DataJson[num].toTop == 'true') { //返回到顶部
            ...
            if ((type.indexOf("bottom") > -1) && _this.pinterestData[num].booleanUp == 0) {
                // CT.alertTip("因版权到期，该内容已经下线，</br>小编正在快马加鞭为你替换新内容！",{time: 5000});
                if (CT.popBooleanUp(_this.pinterestData[num]) == true) {
                    return;
                }
            ...
            if (type.indexOf("bottom") > -1) { //瀑布流
                ...
                if (CT.popBooleanUp(curFocus.TempData) == true) {
                    return;
                }
                // if(curFocus.TempData && curFocus.TempData.booleanUp == 0) {
                // // CT.alertTip("因版权到期，该内容已经下线，</br>小编正在快马加鞭为你替换新内容！",{time: 5000});
                // var bDElement = CT.$("bDPop");
                // if(!bDElement) {
                //     var bDElement = document.createElement("div");
