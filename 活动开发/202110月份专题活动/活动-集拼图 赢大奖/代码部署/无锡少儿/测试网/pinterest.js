...
var page = {
    ...
    getChanceFunc: function () {
        ...
        chance.getChance(parseInt(xjDataLog.getUserId()), 35, function (getActi1Res) {
            if (getActi1Res.data.activityChance >= 1) {
                chance.getChance(parseInt(xjDataLog.getUserId()), 34, function (getActi2Res) {
                    if (getActi2Res.data.activityChance >= 1) {
                        return;
                    } else {
                        chance.setChance(parseInt(xjDataLog.getUserId()), 34, function (setActi2Res) {
                ...
                chance.setChance(parseInt(xjDataLog.getUserId()), 35, function (setActi1Res) {
                    CT.getActivityUrl(35);
                });
            }
        });
    },
    ...
    createTheme: function (callBack) {
        ...
        if (!CT.$('themeListAlert')) {
            ...
            for (var index = 0; index < _this.themeListArr.length; index++) {
                ...
                // themeBtn.style.top = listBtnStartTop + listBtnStartStepC * index + 'px';
